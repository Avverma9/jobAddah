...
const buildPrompt = (scrapedData, hints = {}, mode = "FULL") => {
  const sd = scrapedData ? JSON.stringify(scrapedData, null, 2) : "null";

  // ============== LITE MODE ==============
  if (mode === "LITE") {
    return (
      "TASK: Extract ONLY updates (Dates & Links) for an EXISTING job post.\n\n" +
      "CONTEXT: This is a PATCH update. Ignore static details (Vacancy, Eligibility, Fees).\n\n" +
      "SCHEMA:\n" +
      "{\n" +
      '  "recruitment": {\n' +
      '    "title": "Updated title if explicitly mentioned",\n' +
      '    "importantDates": {\n' +
      '      "examDate": "YYYY-MM-DD or DD-MM-YYYY",\n' +
      '      "admitCardDate": "",\n' +
      '      "resultDate": "",\n' +
      ...
      "}\n\n" +
      "EXTRACTION RULES:\n" +
      "1. TITLE: Extract ONLY if it's a new update (e.g., 'Answer Key Released', 'Result Out')\n" +
  ...
  if (mode === "UPDATE") {
    return (
      "TASK: Extract ONLY changed fields for a job post UPDATE.\n\n" +
      "CONTEXT: Merge this data with existing records. Only extract fields that have CHANGED.\n\n" +
      "SCHEMA:\n" +
      "{\n" +
      '  "recruitment": {\n' +
      '    "importantDates": {\n' +
      ...
      "}\n\n" +
      "RULES:\n" +
      "1. ONLY include fields that have NEW/CHANGED information\n" +
