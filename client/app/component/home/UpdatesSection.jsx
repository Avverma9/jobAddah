...
const PAGE_SIZE = 15;
const FEATURED_CATEGORY_ORDER = ["admissions", "admit-cards", "latest-jobs", "results"];
...
export default function UpdatesSection({ filteredUpdates, onSelectItem }) {
  ...
  const visibleBlocks = useMemo(() => {
    const indexedBlocks = sectionBlocks.map((block, index) => {
      const category = toCanonicalCategory(block.categoryKey || block.key || block.title || "");
      const preferredIndex = FEATURED_CATEGORY_ORDER.indexOf(category);
        ...
        {visibleBlocks.map((block) => {
          ...
          return (
            ...
            >
              ...
              <div
                className="h-[620px] flex-grow space-y-1 overflow-y-auto bg-slate-50/70 p-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
