...
function ViewAllContent() {
  ...
  const sectionName = searchParams.get("name") || "All Posts";
  const sectionLink = searchParams.get("link");
  ...
  useEffect(() => {
    ...
    fetch(`/api/gov-post/view-all?link=${encodeURIComponent(sectionLink)}`)
      .then((res) => res.json())
      .then((payload) => {
        if (payload.success) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white">
        ...
        <p className="text-slate-600 font-medium">Loading {sectionName}...</p>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        {!filteredJobs.length ? (
          <div className="bg-white rounded-xl p-12 text-center border border-slate-200">
            ...
            <h3 className="text-lg font-semibold text-slate-800 mb-1">No posts found</h3>
            <p className="text-slate-500 text-sm">We couldn&apos;t find any posts matching your criteria.</p>
          </div>
        ) : (
