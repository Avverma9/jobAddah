...
export const extractAge = (age, fallbackRelaxation = "") => {
  if (!age || typeof age !== "object")
    return { text: [], relaxation: fallbackRelaxation || "", categoryWise: [] };
  ...
  if (!relaxationText && fallbackRelaxation) {
    relaxationText = fallbackRelaxation;
  }
...
export const extractVacancy = (vacancy) => {
  ...
  if (Array.isArray(vacancy.positions)) {
    positions = vacancy.positions.map((p) => {
      ...
      return {
        name:
          (p.name || p.postName || p.position || "Post") +
          (p.area ? ` (${p.area})` : ""),
        count:
          [
            p.totalPosts,
        ...
        qualification:
          ...
          "Refer to notification for specific details regarding this post.",
        ageLimit: p.ageLimit || "",
        category: p.category || "",
        areaType: p.areaType || "",
