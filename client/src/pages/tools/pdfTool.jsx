...
const CropModal = ({ image, onClose, onCrop }) => {
  ...
  useEffect(() => {
    ...
    imgRef.current.onload = () => {
      // Professional Fit: Calculate scale to fit within 80vw and 70vh
      ...
      const newScale = Math.min(scaleX, scaleY, 1); // Don't upscale, only downscale
  ...
  const handleCropConfirm = () => {
    ...
    // CRITICAL FIX: Map selection from Screen Coordinates -> Original Image Coordinates
    // Previous bug: selection.w * scale (Wrong) -> selection.w / scale (Correct)
    ...
    ctx.drawImage(
      imgRef.current,
      sourceX, sourceY, sourceW, sourceH, // Source (Original Image)
      0, 0, sourceW, sourceH              // Destination (New Canvas)
    );
...
export default function PdfTool() {
  ...
  const getCompressedDataUrl = async (url, q) => {
      return new Promise((resolve) => {
          ...
          img.onload = () => {
              const canvas = document.createElement('canvas');
              const ctx = canvas.getContext('2d');
              // Scale down slightly if quality is very low to help reach target KB
  ...
  const generatePDFBlob = async (returnBlob = false, overrideQuality = null) => {
    if (!window.jspdf || images.length === 0) return;
    setProcessing(true);
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        // Apply compression via Canvas if quality is < 1
  ...
  const autoOptimize = async () => {
    ...
    // Binary searchish approach to find best quality
    for (let step = 0; step < 5; step++) {
        ...
        if (size <= targetBytes) {
            bestQuality = mid;
            // If we are under target, try to go higher to maximize quality
            low = mid; 
             // Keep track of the one that fit closest
            if (targetBytes - size < bestDiff) {
                bestDiff = targetBytes - size;
            }
        } else {
            // If over target, go lower
            high = mid;
        }
    }
          ...
          <div className="flex-1 w-full">
            ...
            {images.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
                    ...
                    <div className="p-3 bg-white border-t flex justify-between items-center">
                        <div className="relative overflow-hidden w-full">
                            ...
                            <input 
                              ...
                              onChange={(e) => e.target.files[0] && replaceImage(img.id, e.target.files[0])}
                            />
                        </div>
                    </div>
                  </div>
                ...
                {/* Add More Card */}
                <label htmlFor="file-upload" className="flex flex-col items-center justify-center h-48 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition bg-transparent opacity-70 hover:opacity-100">
          ...
          <div className="w-full lg:w-80 flex-shrink-0 space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
                ...
                <button 
                    ...
                    className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2 font-bold disabled:opacity-50 disabled:cursor-not-allowed transform active:scale-[0.98]"
                >
                    {processing ? (
                        <>
