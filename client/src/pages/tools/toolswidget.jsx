...
const tools = [
  {
    ...
    icon: <ImageIcon className="w-2 h-3 sm:w-6 sm:h-6" />,
    status: "active",
  ...
  },
  {
    ...
    icon: <FileText className="w-2 h-3 sm:w-6 sm:h-6" />,
    status: "coming_soon",
    color: "red"
  },
  {
    ...
    icon: <Keyboard className="w-2 h-3 sm:w-6 sm:h-6" />,
    status: "coming_soon",
    color: "orange"
  },
  {
    ...
    icon: <Calculator className="w-2 h-3 sm:w-6 sm:h-6" />,
    status: "coming_soon",
    color: "green"
  },
  {
    ...
    icon: <BrainCircuit className="w-2 h-3 sm:w-6 sm:h-6" />,
    status: "coming_soon",
    color: "purple"
  }
];
...
const ToolCard = ({ tool }) => {
  ...
  const containerClasses = "aspect-square w-full rounded-full flex flex-col items-center justify-center text-center transition-all duration-300 relative overflow-hidden p-0.5 sm:p-3";
  ...
  if (isActive) {
    return (
      ...
      >
        ...
        <div className={`p-1 sm:p-2.5 rounded-full mb-0.5 sm:mb-1.5 transition-colors duration-300 ${colorClasses[tool.color]} ring-0 sm:ring-4 ring-white`}>
          {tool.icon}
        </div>
        
        <h3 className="text-[7px] sm:text-sm font-bold text-gray-900 leading-none sm:leading-tight group-hover:text-blue-600 mt-0.5">
  ...
  return (
    <div className={`group ${containerClasses} bg-gray-50 border border-gray-100 sm:border-4 sm:border-gray-100 opacity-90 cursor-not-allowed`}>
      <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-full">
        <span className="flex items-center gap-0.5 sm:gap-1 bg-gray-900 text-white px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[6px] sm:text-[10px] font-medium shadow-xl transform translate-y-1 sm:translate-y-2 group-hover:translate-y-0 transition-transform whitespace-nowrap">
          <Lock className="w-1.5 h-1.5 sm:w-2.5 sm:h-2.5" /> 
          <span className="hidden sm:inline">Coming Soon</span>
          <span className="sm:hidden">Soon</span>
        </span>
      </div>

      <div className={`p-1 sm:p-2.5 rounded-full mb-0.5 sm:mb-1.5 bg-gray-200 text-gray-400 grayscale`}>
        {tool.icon}
      </div>
      
      <h3 className="text-[7px] sm:text-sm font-bold text-gray-700 leading-none sm:leading-tight mt-0.5">
        <span className="sm:hidden">{tool.title}</span>
        <span className="hidden sm:block">{tool.fullTitle}</span>
      </h3>
      ...
      <div className="sm:hidden absolute top-1 right-1 bg-gray-200 rounded-full p-0.5">
          <Lock className="w-1.5 h-1.5 text-gray-500" />
      </div>
    </div>
  );
};
...
export default function Tools() {
  return (
    <div className="w-full bg-slate-50 py-4 sm:py-8 px-2 font-sans overflow-hidden">
      <style>{`
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-6 sm:mb-10 relative z-10">
          <div className="relative inline-block">
            ...
            <h2 className="relative text-xl sm:text-4xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 animate-gradient pb-1 drop-shadow-sm">
              Popular Tools
            </h2>

            {/* Floating Sparkles */}
            <div className="absolute -top-3 -right-4 sm:-top-5 sm:-right-8 animate-float">
               <Sparkles className="w-4 h-4 sm:w-8 sm:h-8 text-yellow-400 fill-yellow-100" />
            </div>
            <div className="absolute -bottom-1 -left-3 sm:-bottom-2 sm:-left-6 animate-float" style={{ animationDelay: '1.5s' }}>
               <Sparkles className="w-3 h-3 sm:w-6 sm:h-6 text-blue-400 fill-blue-100" />
            </div>
          </div>
        </div>

        <div className="flex justify-center items-start gap-1.5 sm:gap-6 px-0.5 sm:px-1">
          {tools.map((tool) => (
            <div key={tool.id} className="flex-1 max-w-[13vw] sm:max-w-[140px]">
              <ToolCard tool={tool} />
            </div>
          ))}
        </div>
