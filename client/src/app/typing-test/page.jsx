...
export default function TypingTest() {
  ...
  const handleChange = (e) => {
    ...
    const accuracyVal =
      val.length > 0 ? Math.round((correctChars / val.length) * 100) : 100;
      ...
      <header className="w-full max-w-5xl flex justify-between items-center mb-12 mt-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">
            JobsAddah Typing <span className="text-indigo-600">Pro</span>
          </h1>
        </div>
      ...
      <main className="w-full max-w-4xl relative">
        ...
        >
          <div className="absolute top-4 right-4 flex gap-4 text-slate-400 font-mono text-sm">
            ...
            <span className="flex items-center gap-1">
              <CheckCircle size={14} /> {accuracy}% ACC
            </span>
          </div>
          ...
          <p className="text-3xl leading-relaxed font-medium tracking-wide text-slate-300 w-full break-words">
            {text.split("").map((char, index) => {
              ...
              if (index < input.length) {
                ...
                bg =
                  "bg-indigo-500 w-0.5 h-8 inline-block animate-pulse align-middle";
                color = "text-indigo-600";
                return (
                  <span
                    key={index}
                    className="relative text-indigo-600 border-l-2 border-indigo-500 animate-pulse"
                  >
                    {char}
                  </span>
                );
              ...
              return (
                <span
                  key={index}
                  className={`${color} ${bg} ${decoration} rounded-sm transition-colors duration-100`}
                >
                  {char}
                </span>
              );
