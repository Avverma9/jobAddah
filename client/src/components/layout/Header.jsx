...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const setHeaderHeightVar = () => {
      try {
        let h = 0;
        if (!hideHeader && headerRef.current) h = headerRef.current.offsetHeight || 0;
            ...
            <div className="flex items-center justify-between h-[70px] md:h-[80px]">
                ...
                <Link href="/" className="flex items-center gap-2 group">
                  ...
                  <div className="flex flex-col">
                    <span className="text-2xl md:text-3xl font-bold text-blue-700 leading-tight tracking-tight">
                      JobsAddah<span className="text-gray-500 text-lg">.com</span>
                    </span>
              ...
              <nav className="hidden md:flex items-center space-x-1 lg:space-x-6">
                ...
                <div className="relative" ref={toolsRef}>
                  <button
                    onClick={() => { setIsToolsOpen(!isToolsOpen); setIsInfoOpen(false); }}
                  ...
                  >
                    <span>Tools</span>
                    <svg className={`w-4 h-4 transition-transform duration-200 ${isToolsOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </button>

                  <div className={`absolute left-1/2 -translate-x-1/2 mt-2 w-64 bg-white rounded-xl shadow-xl ring-1 ring-black/5 py-2 transform transition-all duration-200 origin-top z-50 ${
                ...
                <div className="relative" ref={infoRef}>
                  <button
                    onClick={() => { setIsInfoOpen(!isInfoOpen); setIsToolsOpen(false); }}
                    className={`flex items-center space-x-1 px-3 py-2 rounded-md text-[15px] font-medium transition-colors outline-none ${
                      isInfoOpen
                        ? "text-blue-700 bg-blue-50"
                        : "text-gray-700 hover:text-blue-700 hover:bg-gray-50"
                  ...
                  >
                    <span>Info</span>
                    <svg className={`w-4 h-4 transition-transform duration-200 ${isInfoOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                    </svg>
                  </button>

                  <div className={`absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl ring-1 ring-black/5 py-1 transform transition-all duration-200 origin-top z-50 ${
                    isInfoOpen ? "opacity-100 translate-y-0 visible" : "opacity-0 translate-y-2 invisible"
                  }`}>
                     {[
              ...
              <div className="hidden md:flex items-center gap-3 pl-4">
                {isSignedIn ? (
                  ...
                  <div className="relative" ref={profileRef}>
                    <button onClick={handleProfileClick} className="group flex items-center gap-2 focus:outline-none">
                      <div className="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold border-2 border-transparent group-hover:border-blue-200 transition-all">
                        {user && user.name ? user.name.split(" ").map((s) => s[0]).slice(0, 2).join("").toUpperCase() : "JD"}
                      </div>
                    </button>
                    {isProfileOpen && (
                      <div className="absolute right-0 mt-3 w-60 bg-white rounded-xl shadow-xl ring-1 ring-black/5 py-1 origin-top-right z-50">
                        ...
                        <div className="p-1">
                          <Link href="/saved-jobs" className="block px-3 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-md">Saved Jobs</Link>
                          <Link href="/profile" className="block px-3 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-md">Profile Settings</Link>
                        </div>
                        <div className="border-t border-gray-100 p-1">
                          <button onClick={signOut} className="flex w-full items-center px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-md">Sign out</button>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div ref={mobileMenuRef} className="fixed top-[var(--site-header-height,70px)] left-0 right-0 z-50 bg-white shadow-xl border-t border-gray-100 max-h-[calc(100vh-70px)] overflow-y-auto">
                <div className="px-4 py-4 space-y-2">
                  ...
                  <div className="pt-2 border-t border-gray-100">
                    ...
                    {TOOLS_CONFIG.map((tool) => (
                      <Link key={tool.path} href={tool.path} onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:text-blue-700 hover:bg-blue-50 pl-6">
                        {tool.name}
                      </Link>
                    ))}
                  </div>
                   ...
                   {/* Mobile Info */}
                   <div className="pt-2 border-t border-gray-100">
                    <p className="px-3 py-2 text-xs font-semibold text-gray-500 uppercase">Info</p>
                    {[
                      { label: "About", href: "/about" },
                      { label: "Contact", href: "/contact" },
                      { label: "Terms and Conditions", href: "/terms" },
                      ...
                      <Link key={item.href} href={item.href} onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:text-blue-700 hover:bg-blue-50 pl-6">
                        {item.label}
                      </Link>
                    ))}
                  </div>
                  ...
                  <div className="pt-4 border-t border-gray-100">
                    {!isSignedIn ? (
                      <div className="grid grid-cols-2 gap-3">
                        <button onClick={() => { setAuthOpen(true); setIsMobileMenuOpen(false); }} className="flex justify-center items-center px-4 py-2.5 border border-blue-700 rounded-lg text-base font-medium text-blue-700 hover:bg-blue-50">Login</button>
                        <button onClick={() => { setAuthOpen(true); setIsMobileMenuOpen(false); }} className="flex justify-center items-center px-4 py-2.5 border border-transparent rounded-lg text-base font-medium text-white bg-blue-700 hover:bg-blue-800">Register</button>
                      </div>
                    ) : (
                      <div className="space-y-1">
                        <Link href="/saved-jobs" onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-3 rounded-lg text-base font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-700">Saved Jobs</Link>
                        <Link href="/profile" onClick={() => setIsMobileMenuOpen(false)} className="block px-3 py-3 rounded-lg text-base font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-700">Profile Settings</Link>
                        <button onClick={() => { signOut(); setIsMobileMenuOpen(false); }} className="w-full text-left block px-3 py-3 rounded-lg text-base font-medium text-red-600 hover:bg-red-50">Sign out</button>
                      </div>
                    )}
                  </div>
                </div>
...
function DropdownLink({ href, icon, color, title }) {
  ...
  return (
    <Link href={href} className="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors gap-3">
