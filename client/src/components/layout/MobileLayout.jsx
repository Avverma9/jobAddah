...
export const MobileBottomNav = ({ activeView }) => {
  ...
  const getCurrentView = () => {
    ...
    if (
      path.includes("tools") ||
      path.includes("/pdf-tool") ||
      path.includes("/image-tool") ||
      path.includes("/resume-maker") ||
      path.includes("/typing-test") ||
      path.includes("/quiz-and-earn")
    )
      return "tools";
  ...
  const handleNavClick = (item) => {
    if (item.id === "tools" || item.id === "deadlines") {
      // Navigate to home with query to indicate desired sub-view
      router.push(`/?activeView=${item.id}`);
    } else {
