...
export default function AllAnswerKeys() {
  const dispatch = useDispatch();
  const { answerKeys } = useSelector((state) => state.resource);
  ...
  const items = (answerKeys?.data || []).filter((it) =>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-semibold">All Answer Keys</h2>
      ...
      <div className="overflow-x-auto bg-white rounded-xl border border-slate-200 shadow-sm">
        {answerKeys?.loading && <div className="p-4 text-slate-500">Loading answer keys...</div>}

        {!answerKeys?.loading && items.length === 0 && (
          <div className="p-4 text-slate-400">No answer keys found.</div>
        )}
